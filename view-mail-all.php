...
if (strlen($_SESSION['login']) == 0) {
    ...
    if (NULL !== filter_input(INPUT_POST, 'cancel')) {
        header('location:manage-mail-all.php');
        exit();
    }
    ...
    if (isset($_POST['postmail'])) {
        if (NULL !== filter_input(INPUT_GET, 'id')) {
            ...
            if ($query->rowCount() > 0) {
                ...
                $rc = $stmt->bind_param('iiiiss', $id, $action, $actoff, $loguserId, $actdate, $loguser);
                $stmt->execute();
                $stmt->close();
                $DbConnect->close();
                    ...
                    <form method="post" class="content-wide">
                        ...
                        <div>
                            <table id="vwtable" class="display table table-striped table-bordered table-hover" cellspacing="3" width="100%">
                                ...
                                <tbody>
                                    <?php
                                    $sql = "select * from vwactionstatusfinal ORDER BY ActionDate";
                                    $query = $dbh->prepare($sql);
                                    $query->execute();
                                    $results = $query->fetchAll(PDO::FETCH_OBJ);
                                    $cnt = 1;
