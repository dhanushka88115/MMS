    ...
    if (isset($_POST['postmail'])) {                            //if (NULL !== filter_input(INPUT_POST, 'editmail')) { 
        if (NULL !== filter_input(INPUT_GET, 'id')) {           //if (isset($_GET['id'])) {
            ...
            if ($query->rowCount() > 0) {
                ...
                $sql = "update tblmailmaster set CurrentActionDate=:actdate, CurrentActionId=:action, CurrentActionOfficerId=:actoff, UpdatedBy=:loguser, UpdatedTime=CURRENT_TIMESTAMP() where Id=:id";
                $query = $dbh->prepare($sql);
                $query->bindParam(':id', $id, PDO::PARAM_STR);
            ...
            if ($query->rowCount() > 0) {
                foreach ($results as $result) {
                    ...
                    <form method="post" class="content-wide">
                        ...
                        <h2 style="color: blue;">POST Mail Action</h2>
                        <br>
                        <?php if ($error) { ?>
                        ...
                        <div class="block">
                            ...
                            <select style="background-color: greenyellow; width: 390px" name="fromactoff" id="fromactoff" required="required" onclick="ClearText()"> 
                                ...
                                <?php
                                $sql = "select * from vwuser where Id=:loguserId";
                                $query = $dbh->prepare($sql);
                        ...
                        <div class="block">
                            ...
                            <select name="action" id="action" style="background-color: greenyellow" required="required" onclick="ClearText()"> 
                                <option value="">SELECT</option>
                                <?php
